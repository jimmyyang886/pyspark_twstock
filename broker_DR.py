...
#    print(ExDaterdd.collect())
    ...
    ExDateDF=spark.createDataFrame(ExDaterdd,['exdate', 'count'])
    ExDateDF=ExDateDF.filter("count >= 2")

    #ExDateDF.show(5)

    #dateRDD=ExDateDF.select('date').sort("date").rdd.map(lambda x: x[0].isoformat())
    dateRDD=ExDateDF.select("exdate").sort("exdate").rdd.flatMap(lambda x:x)
    #print(dateRDD.collect())
    #dateRDD.take(5)
    dateRDD.foreach(daily_report)
